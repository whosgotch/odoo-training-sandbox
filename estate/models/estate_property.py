...
class EstateProperty(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            "Expected price of property must be positive.",
        ),
        (
            "check_selling_price",
            "CHECK(selling_price >= 0)",
            "Selling price of property must be positive.",
        ),
    ]
    # Default fields
    ...
    def check_selling_price(self):
        for line in self:
            if (
                not float_is_zero(line.selling_price, precision_rounding=0.01)
                and float_compare(
                    line.selling_price,
                    (line.expected_price / 100 * 90),
